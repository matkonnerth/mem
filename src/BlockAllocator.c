...
BlockAllocator *BlockAllocator_new(size_t defaultBlockSize)
{
    BlockAllocator *allocator =
        (BlockAllocator *)calloc(1, sizeof(BlockAllocator));
    if(!allocator)
    {
        return NULL;
    }
...
char *BlockAllocator_malloc(BlockAllocator *allocator, size_t size)
{
    if ((allocator->current->size + size) > allocator->current->capacity)
    {        
        struct Block *newBlock = Block_new(getBlockSize(size, allocator->defaultBlockSize));
        if (!newBlock)
        {
            return NULL;
        }
    ...
    allocator->current->userPtr = allocator->current->mem + allocator->current->size;
    allocator->current->size += size;
    allocator->current->userSize = size;
    return allocator->current->userPtr;
}

char *BlockAllocator_realloc(BlockAllocator *allocator, size_t size)
{
    if ((allocator->current->size + size) > allocator->current->capacity)
    {
        // we also have to consider the size we have to transfer
        struct Block *newBlock =
            Block_new(getBlockSize(size + allocator->current->userSize*2, allocator->defaultBlockSize));
        if (!newBlock)
        {
            return NULL;
        }
        //we have to copy over the old stuff
        memcpy(newBlock->userPtr, allocator->current->userPtr, allocator->current->userSize);
...
void BlockAllocator_delete(BlockAllocator *allocator)
{
